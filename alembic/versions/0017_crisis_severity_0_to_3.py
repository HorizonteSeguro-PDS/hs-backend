...
def upgrade() -> None:
    ...
    op.drop_constraint(
        "ck_crises_severity_initial", "crises", type_="check"
    )
    op.drop_constraint(
        "ck_crises_severity_calculated", "crises", type_="check"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE crises SET severity_initial = 1 WHERE severity_initial = 0"
    )
    op.execute(
